...
void Slot::init(int unit, int total_columns, int _index) {
	index = _index;
	posX = (M5.Lcd.width() - (SYM_WIDTH * total_columns + PAD_X * (total_columns - 1))) / 2 + unit * (SYM_WIDTH + PAD_X);
...
void Slot::draw(int offsetX, int offsetY) {
	...
	while (y < height) {
		...
		if (y < shadowHeight || y >= height - shadowHeight) {
			...
			M5.Lcd.pushImage(x, y + PAD_Y + offsetY, SYM_WIDTH, sh, buffer);
			shadowY += sh;
			h = sh;
		} else {
			shadowY = 0;
			M5.Lcd.pushImage(x, y + PAD_Y + offsetY, SYM_WIDTH, h, data);
		}
		offset += h;
		y += h;
	}
...
void Slot::flush(uint16_t bgColor, int offsetX, int offsetY) {
	int x = posX + offsetX;
	int y = (M5.Lcd.height() - SYM_HEIGHT) / 2 + offsetY;
...
void Slot::stop(int _acc, int _minVel) {
	if (state == SLOT_START) {
		// If we are forcing a target symbol, we don't want to slow down too much
		// otherwise it takes forever to slide into the forced position.
		if (targetSymbol != -1) {
			acc = _acc * 2; // Decelerate faster initially
			minVel = _minVel * 4; // Keep a higher minimum speed while searching for the target
		} else {
			acc = _acc;
			minVel = _minVel;
		}
...
bool Slot::update() {
	...
	if (vel != 0) {
		...
		if (state == SLOT_DRIFT) {
			...
			// Increase the snapping tolerance window if we are targeting a specific symbol
			// to avoid missing it when traveling at higher minimum speeds.
			int tolerance = (targetSymbol != -1) ? 5 : 2;
			
			if (d <= tolerance || d >= fin - tolerance) {
				int possible_index = (int)roundf(degree / fin);
				if (possible_index >= reelLength) {
					possible_index = 0;
				}
				if (targetSymbol == -1 || symbolIndices[possible_index] == targetSymbol) {
